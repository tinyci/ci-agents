...
func (h *Handler) handleBasic(w http.ResponseWriter, logger *log.SubLogger, event string) bool {
	if event == "" {
		logger.Error("Rejected hook event because the event was missing")
		http.Error(w, "Bad Request", http.StatusBadRequest)
		return true
	}

	if event == eventPing {
		logger.Infof("responding to hook ping from remote host")
		return true
	}
...
func (h *Handler) parseBody(w http.ResponseWriter, logger *log.SubLogger, body []byte, event string) interface{} {
	...
	if !ok {
		logger.Errorf("Could not find converter for event %q", event)
		http.Error(w, "Bad Request", http.StatusBadRequest)
		return nil
	}
	...
	if err != nil {
		logger.Errorf("Rejected hook event because the JSON converter failed: %v", err)
...
func (h *Handler) checkRepo(w http.ResponseWriter, req *http.Request, logger *log.SubLogger, body []byte, obj interface{}, event string) bool {
	...
	if !ok {
		logger.Errorf("Could not find repository reader for event %q", event)
		http.Error(w, "Bad Request", http.StatusBadRequest)
		return true
	}
	...
	if err != nil {
		logger.Errorf("Rejected hook event because we could not read from the repository: %v", err)
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return true
	}

	if repo.HookSecret == "" {
		logger.Error("Rejected hook event because the hook secret is missing")
		http.Error(w, "Bad Request", http.StatusBadRequest)
		return true
	}
	...
	if !h.isValidSignature(body, repo.HookSecret, signature) {
		logger.Error("Rejected hook event because the request signature is invalid")
		http.Error(w, "Bad Request", http.StatusBadRequest)
		return true
	}
...
func (h *Handler) doSubmit(w http.ResponseWriter, logger *log.SubLogger, obj interface{}, event string) bool {
	...
	if !ok {
		logger.Errorf("Could not find dispatcher for event %q", event)
		http.Error(w, "Bad Request", http.StatusBadRequest)
		return true
	}
	...
	if err != nil {
		switch err := err.(type) {
		case *ErrCancelPR:
			logger.WithFields(log.FieldMap{"pull_request": fmt.Sprintf("%v", err.PRID), "repository": err.Repository}).Infof("Canceling PR")
			...
			return false
		default:
			logger.Errorf("Rejected hook event because we could not dispatch the submission: %v", err)
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return true
		}
	}
	...
	go func() {
		now := time.Now()
		if err := h.queueClient.Submit(sub); err != nil {
			logger.Error("Rejected hook event because the submission failed")
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}

		logger.WithFields(log.FieldMap{"submission": fmt.Sprintf("%#v", *sub)}).Infof("Submission took %v", time.Since(now))
	}()
...
func (h *Handler) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	since := time.Now()
	logger.Info("Received hook request")
	...
	go func() { logger.Infof("Full RTT for hook was %v", time.Since(since)) }()
	...
	if req.Body == nil {
		logger.Error("Rejected hook event because the body was missing")
		http.Error(w, "Bad Request", http.StatusBadRequest)
		return
	}
	...
	if err != nil {
		logger.Errorf("Rejected hook event because we could not read the body: %v", err)
		http.Error(w, "Bad Request", http.StatusBadRequest)
		return
	}

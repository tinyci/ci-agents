...
func testTaskToManyRuns(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, runDBTypes, false, runColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, runDBTypes, false, runColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	slice := TaskSlice{&a}
	if err = a.L.LoadRuns(ctx, tx, false, (*[]*Task)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Runs); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.Runs = nil
	if err = a.L.LoadRuns(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Runs); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testTaskToManyAddOpRuns(t *testing.T) {
	var err error
	...
	foreigners := []*Run{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, runDBTypes, false, strmangle.SetComplement(runPrimaryKeyColumns, runColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*Run{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddRuns(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.Runs[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.Runs[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.Runs().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testTaskToOneSubmissionUsingSubmission(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadSubmission(ctx, tx, false, (*[]*Task)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testTaskToOneSetOpSubmissionUsingSubmission(t *testing.T) {
	var err error
...
var (
	taskDBTypes = map[string]string{`ID`: `bigint`, `Status`: `boolean`, `TaskSettings`: `jsonb`, `CreatedAt`: `timestamp with time zone`, `StartedAt`: `timestamp with time zone`, `FinishedAt`: `timestamp with time zone`, `Canceled`: `boolean`, `Path`: `character varying`, `SubmissionID`: `bigint`}
	_           = bytes.MinRead
)

...
func testSubscriptionsExists(t *testing.T) {
	...
	e, err := SubscriptionExists(ctx, tx, o.UserID, o.RepositoryID)
	if err != nil {
...
func testSubscriptionsFind(t *testing.T) {
	...
	subscriptionFound, err := FindSubscription(ctx, tx, o.UserID, o.RepositoryID)
	if err != nil {
		t.Error(err)
	}
...
func testSubscriptionToOneSetOpUserUsingUser(t *testing.T) {
	...
	for i, x := range []*User{&b, &c} {
		...
		if exists, err := SubscriptionExists(ctx, tx, a.UserID, a.RepositoryID); err != nil {
			t.Fatal(err)
		} else if !exists {
...
var (
	subscriptionDBTypes = map[string]string{`UserID`: `bigint`, `RepositoryID`: `bigint`}
	_                   = bytes.MinRead
)

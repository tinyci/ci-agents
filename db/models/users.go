...
var UserColumns = struct {
	...
	Username         string
	Token            string
	LastScannedRepos string
	LoginToken       string
}{
	ID:               "id",
	Username:         "username",
	Token:            "token",
	LastScannedRepos: "last_scanned_repos",
...
type userR struct {
	...
	UserCapabilities  UserCapabilitySlice `boil:"UserCapabilities" json:"UserCapabilities" toml:"UserCapabilities" yaml:"UserCapabilities"`
	UserErrors        UserErrorSlice      `boil:"UserErrors" json:"UserErrors" toml:"UserErrors" yaml:"UserErrors"`
}
...
var (
	userAllColumns            = []string{"id", "username", "token", "last_scanned_repos", "login_token"}
	userColumnsWithoutDefault = []string{"username", "token", "last_scanned_repos", "login_token"}
...
// UserCapabilities retrieves all the user_capability's UserCapabilities with an executor.
func (o *User) UserCapabilities(mods ...qm.QueryMod) userCapabilityQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"user_capabilities\".\"user_id\"=?", o.ID),
	)

	query := UserCapabilities(queryMods...)
	queries.SetFrom(query.Query, "\"user_capabilities\"")

	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"user_capabilities\".*"})
	}

	return query
}

// UserErrors retrieves all the user_error's UserErrors with an executor.
func (o *User) UserErrors(mods ...qm.QueryMod) userErrorQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"user_errors\".\"user_id\"=?", o.ID),
	)

	query := UserErrors(queryMods...)
	queries.SetFrom(query.Query, "\"user_errors\"")

	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"user_errors\".*"})
	}
...
// LoadUserCapabilities allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadUserCapabilities(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`user_capabilities`),
		qm.WhereIn(`user_capabilities.user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load user_capabilities")
	}

	var resultSlice []*UserCapability
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice user_capabilities")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on user_capabilities")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for user_capabilities")
	}
	...
	if singular {
		object.R.UserCapabilities = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &userCapabilityR{}
			}
			foreign.R.User = object
		}
		return nil
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.UserID {
				local.R.UserCapabilities = append(local.R.UserCapabilities, foreign)
				if foreign.R == nil {
					foreign.R = &userCapabilityR{}
				}
				foreign.R.User = local
...
// LoadUserErrors allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadUserErrors(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`user_errors`),
		qm.WhereIn(`user_errors.user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load user_errors")
	}

	var resultSlice []*UserError
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice user_errors")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on user_errors")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for user_errors")
	}
	...
	if singular {
		object.R.UserErrors = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &userErrorR{}
			}
			foreign.R.User = object
		}
		return nil
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.UserID {
				local.R.UserErrors = append(local.R.UserErrors, foreign)
				if foreign.R == nil {
					foreign.R = &userErrorR{}
				}
				foreign.R.User = local
...
// Sets o.R.User's Submissions accordingly.
// Replaces o.R.Submissions with related.
// Sets related.R.User's Submissions accordingly.
func (o *User) SetSubmissions(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Submission) error {
	query := "update \"submissions\" set \"user_id\" = null where \"user_id\" = $1"
	...
	if o.R != nil {
		for _, rel := range o.R.Submissions {
...
// Sets related.R.User.
func (o *User) RemoveSubmissions(ctx context.Context, exec boil.ContextExecutor, related ...*Submission) error {
	var err error
	for _, rel := range related {
		queries.SetScanner(&rel.UserID, nil)
		if rel.R != nil {
...
func (o *User) AddSubscriptions(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Subscription) error {
	...
	for _, rel := range related {
		if insert {
			...
			values := []interface{}{o.ID, rel.UserID, rel.RepositoryID}
...
// AddUserCapabilities adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.UserCapabilities.
// Sets related.R.User appropriately.
func (o *User) AddUserCapabilities(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*UserCapability) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.UserID = o.ID
	...
	if o.R == nil {
		o.R = &userR{
			UserCapabilities: related,
		}
	} else {
		o.R.UserCapabilities = append(o.R.UserCapabilities, related...)
	}

	for _, rel := range related {
		if rel.R == nil {
			rel.R = &userCapabilityR{
				User: o,
			}
		} else {
			rel.R.User = o
...
// AddUserErrors adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.UserErrors.
// Sets related.R.User appropriately.
func (o *User) AddUserErrors(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*UserError) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.UserID = o.ID
	...
	if o.R == nil {
		...
		o.R.UserErrors = append(o.R.UserErrors, related...)
	}

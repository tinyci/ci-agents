...
func testSessionsExists(t *testing.T) {
	...
	e, err := SessionExists(ctx, tx, o.Key)
	if err != nil {
...
func testSessionsFind(t *testing.T) {
	...
	sessionFound, err := FindSession(ctx, tx, o.Key)
	if err != nil {
		t.Error(err)
	}
...
var (
	sessionDBTypes = map[string]string{`Key`: `character varying`, `Values`: `character varying`, `ExpiresOn`: `timestamp with time zone`}
	_              = bytes.MinRead
)

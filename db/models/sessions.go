...
type Session struct {
	...
	Values    string    `boil:"values" json:"values" toml:"values" yaml:"values"`
...
var SessionWhere = struct {
	...
	Values    whereHelperstring
	ExpiresOn whereHelpertime_Time
}{
	Key:       whereHelperstring{field: "\"sessions\".\"key\""},
	Values:    whereHelperstring{field: "\"sessions\".\"values\""},
	ExpiresOn: whereHelpertime_Time{field: "\"sessions\".\"expires_on\""},
}
...
var (
	sessionAllColumns            = []string{"key", "values", "expires_on"}
	sessionColumnsWithoutDefault = []string{"key", "values", "expires_on"}
	sessionColumnsWithDefault    = []string{}
	sessionPrimaryKeyColumns     = []string{"key"}
)
...
func FindSession(ctx context.Context, exec boil.ContextExecutor, key string, selectCols ...string) (*Session, error) {
	sessionObj := &Session{}
	...
	query := fmt.Sprintf(
		"select %s from \"sessions\" where \"key\"=$1", sel,
	)
...
func (o *Session) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"sessions\" WHERE \"key\"=$1"
...
func (o *Session) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindSession(ctx, exec, o.Key)
	if err != nil {
		return err
	}
...
func SessionExists(ctx context.Context, exec boil.ContextExecutor, key string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"sessions\" where \"key\"=$1 limit 1)"

...
func (as *assetsvcSuite) TestReadValidation(c *check.C) {
	c.Assert(as.assetClient.Read(1, ioutil.Discard), check.NotNil)
}
...
func (as *assetsvcSuite) TestLogUpDown(c *check.C) {
	...
	c.Assert(as.assetClient.Write(1, r), check.IsNil)
	f.Close()

	time.Sleep(100 * time.Millisecond)

	buf := bytes.NewBuffer(nil)
	c.Assert(as.assetClient.Read(1, buf), check.IsNil)
...
func (as *assetsvcSuite) TestLogStream(c *check.C) {
	...
	go func() {
		c.Assert(as.assetClient.Write(1, pr1), check.IsNil)
	}()

	go func() {
		c.Assert(as.assetClient.Write(2, pr2), check.IsNil)
	}()
	...
	pw1.Close()
	time.Sleep(100 * time.Millisecond)
	c.Assert(as.assetClient.Read(1, pw2), check.IsNil)
	...
	c.Assert(as.assetClient.Read(1, buf1), check.IsNil)
	c.Assert(as.assetClient.Read(2, buf2), check.IsNil)

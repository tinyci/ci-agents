...
func Logout(pCtx context.Context, h *handlers.H, ctx *gin.Context) (interface{}, int, error) {
	...
	if err := sess.Save(); err != nil {
		return nil, 500, utils.WrapError(err, "could not persist session while logging out")
	}
...
func Login(pCtx context.Context, h *handlers.H, ctx *gin.Context) (interface{}, int, error) {
	...
	err2 := sess.Save()
	if err2 != nil {
		return nil, 500, utils.WrapError(err2, "could not persist session for %s", oauthinfo.Username)
	}
